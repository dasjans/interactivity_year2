const settings = Object.freeze({
  // Key we want to monitor
  key: `f`,
  // ID of element to show backdrop in
  dropElementId: `drop`,
  // How often to update visuals based on state
  updateIntervalMs: 100
});

/**
 * @typedef {{
 * pressed: boolean
*  startPress:number
*  lastRelease:number
*  changeRate:{r:number, g:number, b:number}
*  positionRelativeToMiddle: {l:number, t:number, b:number}}
 * }} state
 */

/** @type state */
let state = Object.freeze({
  pressed: false,
  startPress: 0,
  lastRelease: 0,
  changeRate: { r: 2.5, g: 2.5, b: 2.5 },
  positionRelativeToMiddle: { l: 0, t: 0, b: 0 }
});

/**
 * Where we use the state of the keys
 * @returns 
 */
const use = () => {
  const { key, dropElementId } = settings;
  const { pressed, startPress, changeRate, positionRelativeToMiddle: positionRelativeToMiddle } = state;
  // Get the element we want to show the backdrop in and warn if not found
  const dropElement = document.getElementById(dropElementId);
  if (!dropElement) {
    console.warn(`No element with id: ${dropElementId}`);
    return;
  }
  // Reduce changeRate over time by 10% all the time if not pressed
  console.log(`changeRate: r:${changeRate.r}, g:${changeRate.g}, b:${changeRate.b}`);
  if (!pressed) {
    changeRate.r *= 0.9;
    changeRate.g *= 0.9;
    changeRate.b *= 0.9;
  } else { // If pressed, increase changeRate based on how long it's been pressed
    const pressDuration = performance.now() - startPress;
    // The longer it's been pressed, the faster the changeRate increases
    const rateIncrease = Math.min(pressDuration / 50, 25); // Cap increase to avoid too fast
    changeRate.r += 0.25 * rateIncrease;
    changeRate.g += 0.25 * rateIncrease;
    changeRate.b += 0.25 * rateIncrease;
    // Cap changeRate to avoid it getting too fast
    changeRate.r = Math.min(changeRate.r, 25);
    changeRate.g = Math.min(changeRate.g, 25);
    changeRate.b = Math.min(changeRate.b, 25);
  }
  // If changeRate is very high and not pressed, reduce it quickly and go into negative to create a "bounce back" effect for a short time
  if (!pressed && (changeRate.r > 2 || changeRate.g > 2 || changeRate.b > 2)) {
    changeRate.r -= 55;
    changeRate.g -= 55;
    changeRate.b -= 55;
    // Ensure backdropColor doesn't go below 0
    positionRelativeToMiddle.l = Math.max(positionRelativeToMiddle.l, -25500);
    positionRelativeToMiddle.t = Math.max(positionRelativeToMiddle.t, -25500);
    positionRelativeToMiddle.b = Math.max(positionRelativeToMiddle.b, -25500);
  }
  // Update backdropColor based on changeRate and cap to 0-255 range
  saveState({
    positionRelativeToMiddle: {
      l: Math.min((positionRelativeToMiddle.l + changeRate.r), 25500),
      t: Math.min((positionRelativeToMiddle.t + changeRate.g), 25500),
      b: Math.min((positionRelativeToMiddle.b + changeRate.b), 25500)
    }
  });
  // If any of the values of backdropColor is at max, set backdropColor to (255,10,0) and reset changeRate to (0,0,0)
  /*if (positionRelativeToMiddle.l === 255 || positionRelativeToMiddle.t === 255 || positionRelativeToMiddle.b === 255) {
    saveState({
      positionRelativeToMiddle: { l: 255, t: 10, b: 0 },
      changeRate: { r: 0, g: 0, b: 0 }
    });
    dropElement.style.backgroundColor = `rgb(${Math.floor(positionRelativeToMiddle.l)}, ${Math.floor(positionRelativeToMiddle.t)}, ${Math.floor(positionRelativeToMiddle.b)})`;
    return;
  }*/
  // Update the element's background color
  dropElement.style.left = `calc(50% - 5vmin - ${(Math.floor(positionRelativeToMiddle.l - 128) / 255)}vw)`;
  dropElement.style.top = `calc(50% - 5vmin - ${(Math.floor(positionRelativeToMiddle.t - 128) / 255)}vh)`;
};

/**
 * Key is pressed
 * @param {KeyboardEvent} event 
 * @returns 
 */
const onKeyDown = (event) => {
  const { key } = settings;
  let { pressed, startPress, lastRelease, changeRate, positionRelativeToMiddle } = state;

  // Is it the key we are tracking?
  if (key !== event.key) {
    console.log(`Ignoring keydown for key: ${event.key}`);
    return;
  }
  // Prevent default action (like scrolling for space)
  event.preventDefault();
  // If pressed again within 350ms of last release, ignore it (debounce) and if held down, ignore repeats
  if ((performance.now() - lastRelease) < 350 || event.repeat) {
    console.log(`Ignoring keydown for key: ${event.key} due to debounce or repeat`);
    return;
  }
  // Wasn't pressed before, now it is - keep track of time
  if (!pressed) startPress = performance.now();
  // 
  // Update state
  saveState({
    // We're in keydown, so yes pressed
    pressed: true,
    // Track the time of this event
    startPress
  });
};

/**
 * Key is released
 * @param {KeyboardEvent} event 
 * @returns 
 */
const onKeyUp = (event) => {
  const { key } = settings;

  // Is it a key we're tracking?
  if (key !== event.key) {
    console.log(`Ignoring keyup for key: ${event.key}`);
    return;
  }
  console.log(`onKeyUp`);

  event.preventDefault();

  // Update state
  saveState({
    // Not pressed
    pressed: false,
    // Return to init
    startPress: 0,
    // Track when it was released
    lastRelease: performance.now()
  });
};

/**
 * Listen for key events
 */
function setup() {
  document.addEventListener(`keydown`, onKeyDown);
  document.addEventListener(`keyup`, onKeyUp);

  setInterval(use, settings.updateIntervalMs);
}
setup();

/**
 * Update state
 * @param {Partial<state>} s 
 */
function saveState(s) {
  state = Object.freeze({
    ...state,
    ...s
  });
}
